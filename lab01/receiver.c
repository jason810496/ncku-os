...
void receive(message_t* message_ptr, mailbox_t* mailbox_ptr,
  ...
  */
  ...
  if (communication_method == MESSAGE_PASSING) {
    ...
    memcpy(message_ptr, mailbox_ptr->storage.shared_memory_addr,
           sizeof(message_t));
  } else {
  ...
  update_elapsed_time(timestamp_ptr->start_time, timestamp_ptr->end_time,
                      &timestamp_ptr->elapsed_time);
}
