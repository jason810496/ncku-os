...
#define OSFS_MAX_EXTENTS \
    ((OSFS_BLOCK_SIZE - sizeof(uint32_t)) / sizeof(struct osfs_extent))
#define OSFS_MAX_BLOCKS_PER_EXTENT 8 /* It can be ~(uint32) 0 */
#define OSFS_MAX_SIZES_PER_EXTENT \
    (OSFS_MAX_BLOCKS_PER_EXTENT * OSFS_BLOCK_SIZE)
#define OSFS_MAX_FILESIZE                                          \
    ((uint64_t) OSFS_MAX_BLOCKS_PER_EXTENT * OSFS_BLOCK_SIZE * \
     OSFS_MAX_EXTENTS)
...
/**
 * Struct: osfs_extent
 * Description: Extent structure for extent-based file storage.
 */
struct osfs_extent {
    struct osfs_extent *next; // Pointer to the next extent
    uint32_t block_no;       // Starting block number
};
